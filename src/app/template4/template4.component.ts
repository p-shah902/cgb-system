...
export class Template4Component  implements AfterViewInit{
  ...
  canEditJVAligned(jvReviewUserId: number | null, isJVReviewDone: boolean = false): boolean {
    ...
    // Check if user matches jvReviewUserId (handle both string and number types)
  ...
  canShowUpdateForJVApproval(): boolean {
    ...
    // AND can actually edit the JV Aligned checkbox (not disabled due to isJVReviewDone)
  ...
  hasJVAlignedConsultation(): boolean {
    ...
    // Check if logged-in JV Admin user has any consultation row with JV Aligned checked
    const loggedInUserId = Number(this.loggedInUser.id);
      ...
      if (userId) {
        ...
        if (jvAlignedControl) {
          ...
          if (isJVReviewDone) {
            ...
            // If review is not done, enable/disable based on user permissions
            this.onJVReviewChange(index, userId);
          }
        }
      }
    });
